...
import 'portfolio_management_web.dart';
...
extension ThemeColorsWeb on BuildContext {
...
class PortfolioViewScreenWeb extends StatefulWidget {
  const PortfolioViewScreenWeb({super.key});

  @override
  State<PortfolioViewScreenWeb> createState() => _PortfolioViewScreenWebState();
}

class _PortfolioViewScreenWebState extends State<PortfolioViewScreenWeb>
  ...
  Future<void> _loadPortfolio() async {
    try {
      ...
      if (res2.statusCode == 200) {
        ...
        albums = data["albums"] ?? [];
        items = rawItems.where((i) => !fIds.contains(i["id"].toString())).toList();
  ...
  Future<void> _goEditWorks() async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const PortfolioManagementWeb()),
    );
    _loadPortfolio().then((_) => setState(() {}));
  }
  ...
  Widget _buildLoading() => PhotographerWebShell(
        selectedIndex: 1,
        child: Scaffold(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  "Showcase your work, albums, photos, and videos in one place",
                  style: TextStyle(
            ...
            children: [
              ...
              if (expYears > 0 || price > 0)
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: context.isDark ? _green.withOpacity(0.2) : _greenBg,
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Container(width: 1, height: 28, color: _green.withOpacity(0.15)),
                      _miniStat(Icons.attach_money_rounded, "\$$price/hr", "Rate"),
                      Container(width: 1, height: 28, color: _green.withOpacity(0.15)),
                      _miniStat(Icons.collections_outlined,
                          "${featured.length + items.length}", "Works"),
                    ],
                  ),
                ),
  ...
  Widget _avatarFallback() => Container(
        ...
        child: Center(
          child: Icon(Icons.person_rounded, size: 36, color: _green.withOpacity(0.5)),
        ),
      );
  ...
  Widget _buildEditButtonsVertical() => Column(
        children: [
          _editBtn("Edit Profile", Icons.person_outline_rounded, true, _goEditProfile),
          const SizedBox(height: 10),
          _editBtn("Edit Works", Icons.photo_library_outlined, false, _goEditWorks),
        ],
      );
  ...
  Widget _editBtn(String label, IconData icon, bool filled, VoidCallback onTap) =>
      GestureDetector(
        onTap: onTap,
        child: Container(
          height: 44,
  ...
  Widget _buildStatsStrip() => Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceAround,
          children: [
            _statItem(_allPhotos.length.toString(), "Photos", Icons.photo_outlined, _greenSoft),
            _vDiv(),
            _statItem(_allVideos.length.toString(), "Videos", Icons.videocam_outlined, _greenSoft),
            _vDiv(),
            _statItem(albums.length.toString(), "Albums", Icons.photo_album_outlined, _gold),
            _vDiv(),
            _statItem(featured.length.toString(), "Featured", Icons.star_rounded, _gold),
          ],
        ),
      );

  Widget _statItem(String val, String label, IconData icon, Color color) => Column(
        children: [
  ...
  Widget _vDiv() => Container(height: 30, width: 1, color: _green.withOpacity(0.08));
  ...
  Widget _buildPhotosTab() {
    ...
    return Column(
      ...
      children: [
        ...
        photos.isEmpty ? _buildEmptyState(label: "No photos yet") : _buildWorksGrid(photos),
      ],
    );
  }
  ...
  Widget _buildVideosTab() {
    ...
    return Column(
      ...
      children: [
        ...
        vids.isEmpty ? _buildEmptyState(label: "No videos yet") : _buildVideosGrid(vids),
      ],
    );
  }
  ...
  Widget _buildAlbumsTab() => albums.isEmpty
      ...
      : Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionTitle("${albums.length} Albums", Icons.photo_album_outlined, _grey),
            _buildAlbumsGrid(),
          ],
        );
            ...
            Row(
              children: [
                ...
                if (featured.length > 2) ...[
                  ...
                  Expanded(
                    child: GestureDetector(
                      onTap: () => featured.length <= 3 ? _openMedia(featured[2]) : null,
                      child: Stack(
                        children: [
          ...
          children: [
            isVideo
                ? Container(
                    color: _green.withOpacity(0.08),
                    child: Center(
                      child: Icon(Icons.play_circle_fill, size: 36, color: _greenSoft),
                    ),
                  )
                : url.isNotEmpty
                    ? Image.network(
  ...
  Widget _buildWorksGrid(List data) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        int crossCount = constraints.maxWidth > 1000
            ? 4
                  ...
                  children: [
                    isVideo
                        ? Container(
                            ...
                            child: Center(
                              child: Icon(Icons.play_circle_fill, size: 28, color: _greenSoft),
                            ),
                          )
                        : url.isNotEmpty
                            ? Image.network(
                ...
                child: ClipRRect(
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      Center(
                        child: Icon(Icons.play_circle_fill, size: 48, color: _green.withOpacity(0.35)),
                      ),
                      Container(
                        decoration: const BoxDecoration(
                          gradient: LinearGradient(
  ...
  Widget _buildAlbumsGrid() => LayoutBuilder(
        builder: (context, constraints) {
          final crossCount = constraints.maxWidth > 950 ? 3 : 2;
  ...
  void _openMedia(Map item) {
    ...
    showDialog(
      ...
      builder: (_) => isVideo ? _VideoDialogWeb(item: item) : _PhotoDialogWeb(item: item),
    );
  }
  ...
  Widget _sectionTitleWithAction(String title, IconData icon, VoidCallback onAction) =>
      Padding(
        padding: const EdgeInsets.fromLTRB(0, 24, 0, 12),
        child: Row(
          children: [
  ...
  Widget _placeholder() => Container(
        ...
        child: Center(
          child: Icon(Icons.photo_outlined, size: 26, color: Colors.white.withOpacity(0.18)),
        ),
      );
        ...
        child: Column(
          children: [
            Container(
              ...
              child: Icon(Icons.photo_library_outlined, size: 26, color: _green.withOpacity(0.3)),
            ),
...
class _PhotoDialogWeb extends StatelessWidget {
  final Map item;
  const _PhotoDialogWeb({required this.item});
...
class _VideoDialogWeb extends StatefulWidget {
  final Map item;
  const _VideoDialogWeb({required this.item});

  @override
  State<_VideoDialogWeb> createState() => _VideoDialogWebState();
}

class _VideoDialogWebState extends State<_VideoDialogWeb> {
